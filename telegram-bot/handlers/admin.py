...
from utils.rich import reply_rich, send_rich_message
...
def register(client):
    ...
    async def on_health(event):
        ...
        body = (
            f"<b>Catalogue</b>: {counts['dict']} words, {counts['idioms']} idioms, "
            f"{counts['names']} names\n"
            f"<b>Cache age</b>: {'unknown' if age is None else str(age // 60) + ' minutes'}\n"
            f"<b>Users seen</b>: {users}\n"
            f"<b>Active subscriptions</b>: {subs}\n"
            f"<b>Button tokens</b>: {tokens}\n"
            f"<b>Jobs pending</b>: {scheduler.pending_count()}, failed: {failed}\n"
            f"<b>Outbox pending</b>: {pending_out}\n"
            f"<b>Database</b>: {size_mb:.1f} MB\n"
            f"<b>Last database error</b>: {esc(supabase.last_error or 'none')}"
        )
        await reply_rich(event, title="Health", body=body)
    ...
    async def on_broadcast(event):
        ...
        if not message:
            await reply_rich(
                ...
                body="Send /broadcast followed by the message. It goes to every active "
        ...
        if not targets:
            await reply_rich(event, title="No recipients",
                             body="Nobody has an active subscription, so there is nobody to send to.")
            return
        ...
        await reply_rich(
            ...
            body=f"{len(targets)} recipients, sent {config.BROADCAST_CHUNK} at a time. "
                 f"The queue survives a restart, so an interrupted send resumes rather than "
                 f"starting over.",
        )
    ...
    async def on_refresh(event):
        ...
        await reply_rich(
            event, title="Catalogue refreshed",
            body=", ".join(f"{entries.LABELS[tab]}: {count}" for tab, count in counts.items()),
        )
...
async def broadcast_chunk(client, payload):
    ...
    for row in rows:
        try:
            await send_rich_message(client, row["chat_id"], title="A note from the maintainer",
                                    body=esc(row["body"]))
            db.execute(
    ...
    if remaining:
        ...
        if config.BOT_OWNER_ID:
            ...
            await send_rich_message(
                client, config.BOT_OWNER_ID, title="Broadcast finished",
                body=f"Delivered to {sent} chats, {failed} failed.",
            )
