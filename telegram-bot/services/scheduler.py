...
def job(kind: str):
    """Register a job handler. Handlers take (client, payload: dict)."""
...
def pending_count() -> int:
    return db.scalar("SELECT COUNT(*) FROM jobs WHERE status = 'pending'", (), 0)
...
async def worker(client) -> None:
    """Long running loop. Started once from bot.py."""
    global _client
    _client = client
