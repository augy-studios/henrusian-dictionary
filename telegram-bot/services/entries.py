...
Search behaviour deliberately matches main-site/script.js so the bot and the web app
...
def find_anywhere(entry_id: str) -> tuple[Optional[str], Optional[dict]]:
    for tab in TAB_ORDER:
        entry = find(tab, entry_id)
        if entry:
            return tab, entry
    return None, None
...
def word_of_the_day(on: Optional[date] = None) -> Optional[dict]:
    """Deterministic per calendar date, so every subscriber sees the same entry and the
